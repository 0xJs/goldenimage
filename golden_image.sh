...
apt_update (){
  ...
  echo ---------------------------------------------------------------------------;
  echo Updating;
  # Redirect output of update to a logfile
  apt update -y &>> "$updatelog";
  echo Upgrading;
  # Redirect output of upgrade to a logile
  apt full-upgrade -y &>> "$updatelog";
}

# Make directory logs in the path where the script is, if it does not exist yet
...
while getopts ':uigr' opt; do
  case "$opt" in
    u)
      ...
      for repos in $(cat $listhpath/githubrepos.txt)
        do
          ...
          if [ -d /opt/$localrepodir ] ; then
            ...
            git pull &>> "$gitlog";
            echo $localrepodir exists, updated tool with git pull;
          else
